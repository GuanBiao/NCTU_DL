...
//-------------TO DO--------------------------------
			if (i == eb.size() - 1) {					// the last experience!!
                               error = 0 - sp.evaluate_score();
			}
			else {
                                int a_next = get_best_move(spp);
                                int r_next = spp.move(a_next);
                                error = r_next + spp.evaluate_score() - sp.evaluate_score();
			}
//--------------------------------------------------
...
int main(int argc, const char* argv[]) {
	...
	*/
	...
	for (int n = 0; n < 1000000; n++) {
		...
		/*
		std::ofstream out;
                std::string save = "Test/0456616.weight";
