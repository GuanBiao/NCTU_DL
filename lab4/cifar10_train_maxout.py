...
#   NUM_UNITS: the number of maxout units.
...
""" Use maxout activation function. """
...
def maxout(features):
  ...
  shape[-1] //= NUM_UNITS
  shape += [NUM_UNITS]
  return tf.reduce_max(tf.reshape(features, shape), axis=-1, keep_dims=False)
...
def inference(x):
  # conv1
  if USE_HE_METHOD:
    W_stddev = math.sqrt(2.0 / (5 * 5 * 192 * NUM_UNITS))
    b_stddev = 0.0
  else:
    W_stddev = 0.01
    b_stddev = 0.01
  W_conv1 = weight_variable([5, 5, 3, 192 * NUM_UNITS], W_stddev)    # filter = 5x5, in = 3, out = 192 * NUM_UNITS
  if USE_BN:
    h_conv1 = maxout(batch_norm(conv2d(x, W_conv1)))                 # out = 32x32x192
  else:
    b_conv1 = bias_variable([192 * NUM_UNITS], b_stddev)
    h_conv1 = maxout(conv2d(x, W_conv1) + b_conv1)                   # out = 32x32x192

  # mlp1-1
  if USE_HE_METHOD:
    W_stddev = math.sqrt(2.0 / (1 * 1 * 160 * NUM_UNITS))
    b_stddev = 0.0
  else:
    W_stddev = 0.05
    b_stddev = 0.05
  W_mlp1_1 = weight_variable([1, 1, 192, 160 * NUM_UNITS], W_stddev) # filter = 1x1, in = 192, out = 160 * NUM_UNITS
  if USE_BN:
    h_mlp1_1 = maxout(batch_norm(conv2d(h_conv1, W_mlp1_1)))         # out = 32x32x160
  else:
    b_mlp1_1 = bias_variable([160 * NUM_UNITS], b_stddev)
    h_mlp1_1 = maxout(conv2d(h_conv1, W_mlp1_1) + b_mlp1_1)          # out = 32x32x160

  # mlp1-2
  if USE_HE_METHOD:
    W_stddev = math.sqrt(2.0 / (1 * 1 * 96 * NUM_UNITS))
    b_stddev = 0.0
  else:
    W_stddev = 0.05
    b_stddev = 0.05
  W_mlp1_2 = weight_variable([1, 1, 160, 96 * NUM_UNITS], W_stddev)  # filter = 1x1, in = 160, out = 96 * NUM_UNITS
  if USE_BN:
    h_mlp1_2 = maxout(batch_norm(conv2d(h_mlp1_1, W_mlp1_2)))        # out = 32x32x96
  else:
    b_mlp1_2 = bias_variable([96 * NUM_UNITS], b_stddev)
    h_mlp1_2 = maxout(conv2d(h_mlp1_1, W_mlp1_2) + b_mlp1_2)         # out = 32x32x96
  ...
  if USE_HE_METHOD:
    ...
    W_stddev = 0.05
    b_stddev = 0.05
  W_conv2 = weight_variable([5, 5, 96, 192 * NUM_UNITS], W_stddev)   # filter = 5x5, in = 96, out = 192 * NUM_UNITS
  if USE_BN:
    h_conv2 = maxout(batch_norm(conv2d(h_drop, W_conv2)))            # out = 16x16x192
  else:
    b_conv2 = bias_variable([192 * NUM_UNITS], b_stddev)
    h_conv2 = maxout(conv2d(h_drop, W_conv2) + b_conv2)              # out = 16x16x192

  # mlp2-1
  if USE_HE_METHOD:
    W_stddev = math.sqrt(2.0 / (1 * 1 * 192 * NUM_UNITS))
    b_stddev = 0.0
  else:
    W_stddev = 0.05
    b_stddev = 0.05
  W_mlp2_1 = weight_variable([1, 1, 192, 192 * NUM_UNITS], W_stddev) # filter = 1x1, in = 192, out = 192 * NUM_UNITS
  if USE_BN:
    h_mlp2_1 = maxout(batch_norm(conv2d(h_conv2, W_mlp2_1)))         # out = 16x16x192
  else:
    b_mlp2_1 = bias_variable([192 * NUM_UNITS], b_stddev)
    h_mlp2_1 = maxout(conv2d(h_conv2, W_mlp2_1) + b_mlp2_1)          # out = 16x16x192
  ...
  if USE_HE_METHOD:
    ...
    W_stddev = 0.05
    b_stddev = 0.05
  W_mlp2_2 = weight_variable([1, 1, 192, 192 * NUM_UNITS], W_stddev) # filter = 1x1, in = 192, out = 192 * NUM_UNITS
  if USE_BN:
    h_mlp2_2 = maxout(batch_norm(conv2d(h_mlp2_1, W_mlp2_2)))        # out = 16x16x192
  else:
    b_mlp2_2 = bias_variable([192 * NUM_UNITS], b_stddev)
    h_mlp2_2 = maxout(conv2d(h_mlp2_1, W_mlp2_2) + b_mlp2_2)         # out = 16x16x192
  ...
  # conv3
  if USE_HE_METHOD:
    W_stddev = math.sqrt(2.0 / (3 * 3 * 192 * NUM_UNITS))
    b_stddev = 0.0
  else:
    W_stddev = 0.05
    b_stddev = 0.05
  W_conv3 = weight_variable([3, 3, 192, 192 * NUM_UNITS], W_stddev)  # filter = 3x3, in = 192, out = 192 * NUM_UNITS
  if USE_BN:
    h_conv3 = maxout(batch_norm(conv2d(h_drop, W_conv3)))            # out = 8x8x192
  else:
    b_conv3 = bias_variable([192 * NUM_UNITS], b_stddev)
    h_conv3 = maxout(conv2d(h_drop, W_conv3) + b_conv3)              # out = 8x8x192
  ...
  if USE_HE_METHOD:
    ...
    W_stddev = 0.05
    b_stddev = 0.05
  W_mlp3_1 = weight_variable([1, 1, 192, 192 * NUM_UNITS], W_stddev) # filter = 1x1, in = 192, out = 192 * NUM_UNITS
  if USE_BN:
    h_mlp3_1 = maxout(batch_norm(conv2d(h_conv3, W_mlp3_1)))         # out = 8x8x192
  else:
    b_mlp3_1 = bias_variable([192 * NUM_UNITS], b_stddev)
    h_mlp3_1 = maxout(conv2d(h_conv3, W_mlp3_1) + b_mlp3_1)          # out = 8x8x192

  # mlp3-2
  if USE_HE_METHOD:
    W_stddev = math.sqrt(2.0 / (1 * 1 * 10 * NUM_UNITS))
    b_stddev = 0.0
  else:
    W_stddev = 0.05
    b_stddev = 0.05
  W_mlp3_2 = weight_variable([1, 1, 192, 10 * NUM_UNITS], W_stddev)  # filter = 1x1, in = 192, out = 10 * NUM_UNITS
  if USE_BN:
    h_mlp3_2 = maxout(batch_norm(conv2d(h_mlp3_1, W_mlp3_2)))        # out = 8x8x10
  else:
    b_mlp3_2 = bias_variable([10 * NUM_UNITS], b_stddev)
    h_mlp3_2 = maxout(conv2d(h_mlp3_1, W_mlp3_2) + b_mlp3_2)         # out = 8x8x10
...
def evaluate(sess, accuracy, summary_op_eval):
  ...
  for i in xrange(50):
    acc_, summary = sess.run([accuracy, summary_op_eval],
                             feed_dict={x: Xte[i * 200 : (i + 1) * 200],
                                        y: Yte[i * 200 : (i + 1) * 200],
                                        keep_prob: 1.0, train_phase: False})
    acc += acc_

  return acc / 50.0, summary
