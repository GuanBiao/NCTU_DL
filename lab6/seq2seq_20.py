...
train_length = 20
test_length = [10, 20, 30]
seq_length = 30 # Max length for training or test.
word_size = 256
batch_size = 64
hidden_size = 500
embedding_size = 100
...
train_op = tf.train.RMSPropOptimizer(learning_rate, momentum=0.9, epsilon=1e-6).minimize(loss, global_step=global_step)
...
for i in range(iteration):
  length = np.random.randint(1, train_length + 1)
  _, feed_dict = get_batch(length)
  feed_dict.update({target_weights[i] : 1.0 for i in range(length)})
  feed_dict.update({target_weights[i] : 0.0 for i in range(length, seq_length)})
