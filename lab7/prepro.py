...
def main():
    ...
    with tf.Session() as sess:
        ...
        for split in ['train', 'val', 'test']:
            ...
            if split == 'train':
                slices = 10
                examples_per_slice = n_examples / slices
                for i in range(slices):
                    small_save_path = './data/%s/%s.features.%d.hkl' % (split, split, i)
                    if i == slices - 1:
                        j = n_examples
                    else:
                        j = (i + 1) * examples_per_slice
                    small_image_path = image_path[i*examples_per_slice : j]
                    small_n_examples = len(small_image_path)
                    small_feats = np.ndarray([small_n_examples, 196, 512], dtype=np.float32)

                    for start, end in zip(range(0, small_n_examples, batch_size),
                                          range(batch_size, small_n_examples + batch_size, batch_size)):
                        image_batch_file = small_image_path[start:end]
                        image_batch = np.array(map(lambda x: ndimage.imread(x, mode='RGB'), image_batch_file)).astype(
                            np.float32)
                        feats = sess.run(vggnet.features, feed_dict={vggnet.images: image_batch})
                        small_feats[start:end, :] = feats
                        print ("Processed %d %s features.." % (end, split))

                    # use hickle to save huge feature vectors
                    hickle.dump(small_feats, small_save_path)
                    print ("Saved %s.." % (small_save_path))
            else:
