...
class CaptionGenerator(object):
    def __init__(self, word_to_idx, dim_feature=[196, 512], dim_embed=512, dim_hidden=1024, n_time_step=16, 
                  prev2out=True, ctx2out=True, alpha_c=0.0, selector=True, dropout=True, attention='soft'):
        """
        Args:
    ...
    def _attention_layer(self, features, features_proj, h, reuse=False):
        with tf.variable_scope('attention_layer', reuse=reuse):
            ...
            if self.attention == 'soft':
                ...
                dist = tf.contrib.distributions.Multinomial(total_count=1., probs=alpha)
                samples = dist.sample(sample_shape=1)[0]
                context = tf.reduce_sum(features * tf.expand_dims(samples, 2), 1, name='context')
                alpha = alpha * samples
