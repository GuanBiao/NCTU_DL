    ...
    def train(self):
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                # train dataset
                slices = np.random.randint(0, 10)
                self.data = load_coco_data(data_path='./data', split='train', slices=slices)
                total_examples = 82783
                examples_per_slice = 8278
                features = self.data['features']     # (examples_per_slice, 196, 512)
                captions = self.data['captions']     # (399998, 17)
                image_idxs = self.data['image_idxs'] # (399998, )

                start = np.nonzero(image_idxs == slices * examples_per_slice)[0][0]
                end = np.nonzero(image_idxs == (slices + 1) * examples_per_slice)[0][0] if slices < 9 else image_idxs.shape[0]
                ...
                rand_idxs = np.random.permutation(range(start, end))
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    features_batch = features[image_idxs_batch-slices*examples_per_slice]
